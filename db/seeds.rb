...
15.times do
  user = User.create(email: Faker::Internet.email, password: "memphis")
	treasurer = Treasurer.create(first_name: Faker::Name.first_name, last_name: Faker::Name.last_name, date_of_birth: Faker::Date.birthday, branch_id: (1..5).to_a.sample, council: Faker::Address.city, whatsapp_number: Faker::PhoneNumber.cell_phone_with_country_code, employment_status: true, education_level: edu_level.sample, born_again: true, ud_join: Faker::Date.between(from: 32.years.ago, to: 5.years.ago), bishop_podcast: true, year_treasurer: Faker::Date.between(from: 4.years.ago, to: Date.today), treasurer_type: treasurer_type.sample, training_manual: true, tithe: (0..2).to_a.sample, user_id: user.id)
