class CouncilConnection < ActiveRecord::Base
  ...
  self.table_name = "COUNCILS"
	...
	def generate_report(start_date = Date.today.beginning_of_month, end_date = Date.today.end_of_month)
		...
		principal_treasurers = User.joins(:treasurer).where(treasurers: {COUNCIL:self.COUNCIL}, role: User.roles[:principal_treasurer]).pluck(:email)
		puts "No principal treasurers exist for #{self.COUNCIL}" && return if principal_treasurers.blank?
		...
		puts "No data to email" && return if (who_counted.blank? && when_counted.blank? && when_paid.blank?)
